...
%put processes into bins based on how many services they offer
serviceProvidersByServiceCount :-
  setof(Proc,
    Service^Conn^(
  ...
  setof(Service,
    %this syntax tells prolog not to consider Proc, Dir, or Conn when determining if the match is unique
    %I only want unique services, so I did not tell prolog to ignore Service when determining uniqueness.
    Conn^(
      dynamic_service_observation(scanned_proc(P),direction("receiving"),service(Service),Conn);
      dynamic_service_observation(scanned_proc(P),direction("no_connections"),service(Service),Conn)
    ),
    ServiceList),
  length(ServiceList,ServiceCount),
  write(ServiceCount),write(","),writeln(P),
  fail.

%list and count all unique service names
getServiceNames :-
  %setof(ServiceName,dynamic_service_observation(_,_,service(ServiceName),_),ServiceList),
  setof(Service,
    %this syntax tells prolog not to consider Proc, Dir, or Conn when determining if the match is unique
    %I only want unique services, so I did not tell prolog to ignore Service when determining uniqueness.
    Proc^Dir^Conn^(
      dynamic_service_observation(Proc,Dir,service(Service),Conn)
    ),
    ServiceList),
  member(S,ServiceList),
  writeln(S),
  fail.
...
getSendingServiceNames :-
  ...
  setof(Service,
    ...
    %I only want unique services, so I did not tell prolog to ignore Service when determining uniqueness.
    Proc^Conn^(
      dynamic_service_observation(Proc,direction("sending"),service(Service),Conn)
    ),
    ServiceList),
  member(S,ServiceList),
  writeln(S),
...
getRecServiceNames :-
  ...
  setof(Service,
    ...
    %I only want unique services, so I did not tell prolog to ignore Service when determining uniqueness.
    Proc^Conn^(
      dynamic_service_observation(Proc,direction("receiving"),service(Service),Conn)
    ),
    ServiceList),
  member(S,ServiceList),
  writeln(S),
...
%are all service names associated with launchd?
% simple approach would be to count unique services with launchd and compare number to counting all unique services detected
%RESULT: this query seems to confirm that all of the services detected by procexp are also linked to launchd
getLaunchdServiceNames :-
  %setof(ServiceName,dynamic_service_observation(_,_,service(ServiceName),_),ServiceList),
  setof(Service,
    %this syntax tells prolog not to consider Proc, Dir, or Conn when determining if the match is unique
    %I only want unique services, so I did not tell prolog to ignore Service when determining uniqueness.
    Dir^Conn^(
      dynamic_service_observation(scanned_proc("/sbin/launchd"),Dir,service(Service),Conn)
    ),
    ServiceList),
  member(S,ServiceList),
  writeln(S),
...
%number of connections per receiving service
countReceivingConnections :-
  dynamic_service_observation(_,direction("receiving"),service(Service),connections(ConnList)),
  length(ConnList,ConnLength),
  write(ConnLength),write(","),writeln(Service),
  fail.


%list and count unique processes that offer services?
getServiceProviders :-
  setof(Proc,
    Service^Conn^(
      dynamic_service_observation(scanned_proc(Proc),direction("receiving"),Service,Conn)
